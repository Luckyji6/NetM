...
- **普通 USB-C**（非雷电）两台电脑对连一般**不会**出现网卡，这条路径还不完整（串口传输停在 `wip/serial-transport` 分支）。
...
- 客机停在「发现宿主机中」：先重启宿主机上的 `netm host`（线已经插上再开），或用 `--host` 指定 `fe80` 地址。检查系统防火墙是否拦了未签名二进制的入站 UDP 27777 / TCP 27778。
